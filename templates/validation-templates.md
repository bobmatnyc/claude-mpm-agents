...
| Railway | railway-ops | Read deployment logs, check health endpoint, verify database connections |
| GCP/Cloud Run | gcp-ops | Check Cloud Run logs, verify service status, test endpoints |
| AWS | aws-ops | CloudWatch logs, Lambda status, API Gateway tests |
...
| Railway Deploy | Service healthy + fetch tests | Logs + endpoint response | railway-ops (MUST verify) |
| GCP Deploy | Cloud Run active + endpoint tests | Service logs + HTTP 200 | gcp-ops (MUST verify) |
